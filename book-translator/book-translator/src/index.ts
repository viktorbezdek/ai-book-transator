...
program
    ...
    .requiredOption('-f, --file <path>', 'Path to the book file (epub, pdf, docx, or txt)')
    .requiredOption('-k, --api-key <key>', 'Anthropic API key')
    .option('-l, --language <lang>', 'Target language', CONFIG.DEFAULT_TARGET_LANGUAGE)
    .option('-o, --output <path>', 'Output file path')
    .action(async (options) => {
        try {
            const parser = new DocumentParser();
            const coordinator = new TranslationCoordinator(options.apiKey, options.language);
